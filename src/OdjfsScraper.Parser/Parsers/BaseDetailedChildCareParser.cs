...
namespace OdjfsScraper.Parser.Parsers
{
    ...
    {
        public BaseDetailedChildCareParser(ILogger<BaseDetailedChildCareParser<T>> logger) : base(logger)
        {
        }
        ...
        protected override void PopulateFields(T childCare, IDictionary<string, string> details)
        {
            ...
            if (sutqRating != null && Regex.Match(sutqRating, @"[^\*]").Success)
            {
                ...
                _logger.LogError(exception.Message + " Value: '{sutqRating}'");
                throw exception;
            }
        ...
        private Tuple<bool, DateTime?, DateTime?> GetHoursOfOperation(IDictionary<string, string> details, string key)
        {
            ...
            if (!match.Success)
            {
                ...
                _logger.LogError(exception.Message + " Key: '{key}', Value: '{value}'", key, value);
                throw exception;
            }
            ...
            if (!string.IsNullOrEmpty(endTimeString))
            {
                ...
                if (beginTime >= endTime)
                {
                    ...
                    _logger.LogError(
                        exception.Message + " Key: '{key}', Value: '{value}', BeginTime: '{beginTime}', EndTime: '{endTime}'",
                        key,
                        value,
                        beginTime,
                        endTime);
        ...
        private bool GetDetailBool(IDictionary<string, string> details, params string[] keys)
        {
            ...
            _logger.LogError(exception.Message + " Keys: '{keys}', Value: '{value}'", string.Join(", ", keys), value);
            throw exception;
        }
    }
}